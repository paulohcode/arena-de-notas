...
@php
    $arenaTab = request('tab', old('tab', 'turma'));
...
<nav class="flex flex-wrap gap-2 mb-6" aria-label="Abas da arena">
    <a class="tab-btn game-btn-ghost" href="{{ route('admin.areas.arena', ['area' => $area, 'tab' => 'turma']) }}"
        data-active="{{ $arenaTab === 'turma' ? 'true' : 'false' }}">
        Arena da turma
    </a>
    <a class="tab-btn game-btn-ghost" href="{{ route('admin.areas.arena', ['area' => $area, 'tab' => 'turmas']) }}"
        data-active="{{ $arenaTab === 'turmas' ? 'true' : 'false' }}">
        Arena das turmas
        @if($pendingDuels->isNotEmpty())
            <span class="ml-1 text-violet-300">{{ $pendingDuels->count() }}</span>
        @endif
    </a>
</nav>
...
@if($arenaTab === 'turma')
    <div class="game-card p-5">
        ...
        <p class="text-sm text-amber-100/60 mb-4">Cada turma tem a própria agenda semanal de duelos. Abra a turma para configurar dias, espera e limite.</p>
        <div class="space-y-2">
            @forelse($area->classes as $class)
                <div class="flex flex-wrap items-center justify-between gap-2 py-2 border-b border-purple-900/40 text-sm">
                    <span>
                        <a class="hover:text-amber-300" href="{{ route('teacher.classes.show', ['schoolClass' => $class, 'tab' => 'arena', 'arena_tab' => 'turma']) }}">{{ $class->name }}</a>
...
@else
        ...
        @if($area->isRealmArenaOpen())
            <form method="POST" action="{{ route('admin.areas.arena.close', $area) }}">
                ...
                <button class="game-btn-ghost" type="submit">
                    {{ $area->hasRealmArenaSchedule() ? 'Fechar hoje ('.\App\Support\ArenaSchedule::todayLabel().')' : 'Fechar arena entre turmas' }}
                </button>
            </form>
        @else
            <form method="POST" action="{{ route('admin.areas.arena.open', $area) }}">
                ...
                <button class="game-btn" type="submit">
                    {{ $area->hasRealmArenaSchedule() ? 'Abrir hoje ('.\App\Support\ArenaSchedule::todayLabel().')' : 'Abrir arena entre turmas' }}
                </button>
            </form>
        @endif
    </div>
    ...
    <form method="POST" action="{{ route('admin.areas.arena.update', $area) }}" class="game-card p-5 mb-6 space-y-4">
        @csrf
        @method('PUT')
        <input type="hidden" name="tab" value="turmas">
        <div>
            <h2 class="font-display text-xl text-amber-200">Configurações da arena entre turmas</h2>
            <p class="text-sm text-amber-100/60 mt-1">Para cada dia da semana, defina se o reino aceita duelos entre turmas, a espera entre desafios e o limite de {{ \App\Models\GameCurrency::label('auras') }}.</p>
        </div>
        @include('partials.arena-weekday-settings', [
            'prefix' => 'realm_days',
            'week' => $area->realmArenaWeek(),
