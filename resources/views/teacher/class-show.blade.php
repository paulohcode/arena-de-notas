        ...
        <form method="POST" action="{{ route('teacher.teams.store', $class) }}" class="game-card p-5 mb-6 grid md:grid-cols-2 gap-3">
            ...
            <div class="md:col-span-2 flex flex-wrap gap-3">
                @forelse($class->students as $student)
                    @if(! isset($guildMemberships[$student->id]))
            ...
            @foreach($class->teams as $team)
                <form method="POST" action="{{ route('teacher.teams.update', [$class, $team]) }}" class="game-card p-5 space-y-3">
                    ...
                    <div class="flex flex-wrap gap-3">
                        @foreach($class->students as $student)
                            @php
                                $alreadyInOtherGuild = isset($guildMemberships[$student->id]) && ! $team->members->contains($student->id);
                            @endphp
                            @if($alreadyInOtherGuild)
                                <span class="text-sm text-amber-100/40">{{ $student->name }} · {{ $guildMemberships[$student->id] }}</span>
                            @else
                                <label class="text-sm flex items-center gap-2">
                                    <input type="checkbox" name="members[]" value="{{ $student->id }}" @checked($team->members->contains($student->id))>
                                    {{ $student->name }}
                                </label>
                            @endif
                        @endforeach
                ...
                <form method="POST" action="{{ route('teacher.teams.destroy', [$class, $team]) }}" class="-mt-2 mb-4">
                    @csrf @method('DELETE')
