...
@section('content')
<div class="mb-6">
    <a href="{{ route('teacher.classes.show', ['schoolClass' => $class, 'tab' => 'eventos']) }}" class="game-btn-ghost text-sm">← Voltar</a>
...
<div class="flex flex-wrap gap-2 mb-6">
    @if($event->isLiveMode() && in_array($event->status, ['draft', 'scheduled'], true))
        <form method="POST" action="{{ route('teacher.events.start', [$class, $event]) }}">
            @csrf
            <button class="game-btn" type="submit">Iniciar ao vivo</button>
        </form>
    @endif
    @if(! $event->isClosed())
        <form method="POST" action="{{ route('teacher.events.close', [$class, $event]) }}">
            @csrf
    ...
    <div class="game-card p-5">
        ...
        <ol class="space-y-3 text-sm">
            @foreach($event->questions as $question)
                <li>
                    <p class="font-semibold text-amber-100">{{ $question->position + 1 }}. {{ $question->prompt }}</p>
                    <p class="text-purple-200/60">Certa: {{ $question->options[$question->correct_index] ?? '—' }}</p>
                </li>
            @endforeach
        </ol>
        @if($event->prizeItem)
            <p class="mt-4 text-sm text-violet-200">Prêmio: {{ $event->prizeItem->icon }} {{ $event->prizeItem->name }}</p>
        @endif
    </div>
    <div class="game-card p-5">
        ...
        @forelse($ranking as $index => $attempt)
            <div class="flex justify-between border-b border-purple-900/40 py-2 text-sm">
                <span>{{ $index + 1 }}. {{ $attempt->student?->arenaName() ?: $attempt->student?->name }}
                    @if($event->isRealmEvent())
                        <span class="text-purple-300/60">({{ $attempt->schoolClass?->name }})</span>
                    @endif
                </span>
                <span>{{ $attempt->correct_count }} acertos · {{ number_format($attempt->correct_time_ms / 1000, 1) }}s</span>
            </div>
        @empty
            <p class="text-sm text-purple-200/60">Ninguém terminou ainda.</p>
        @endforelse
    </div>
</div>
@endsection
