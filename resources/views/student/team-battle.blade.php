...
@php
    ...
    $viewerFought = in_array($student->id, $log['fighter_ids'] ?? [], true);
        ...
        <div class="game-card p-4 flex flex-wrap gap-3 text-sm">
            @foreach($matchups as $matchup)
                ...
                >
                    ...
                    <p class="text-xs mt-1 {{ ($matchup['winner_id'] ?? null) === ($left?->id) ? 'text-emerald-300' : 'text-rose-300' }}">
                        Venceu: {{ ($matchup['winner_id'] ?? null) === ($left?->id) ? ($left?->arenaName() ?: $left?->name) : ($right?->arenaName() ?: $right?->name) }}
                    </p>
                </div>
            @endforeach
        </div>

        <template x-if="battle && !seriesFinished">
            <div class="space-y-4">
                <div
                ...
                >
                    ...
                    <div class="relative z-10 grid grid-cols-[1fr_auto_1fr] items-end gap-2 md:gap-6 px-3 md:px-8 pt-10 pb-8 min-h-[360px] md:min-h-[460px]">
                        ...
                        <div class="relative self-center w-16 md:w-24 h-24 md:h-32 flex items-center justify-center">
                            <p class="font-display text-xl md:text-3xl text-amber-200/40" x-show="!battle.finished">VS</p>
                        </div>
                    ...
                    <div class="relative z-10 border-t border-amber-300/15 px-4 py-3 flex flex-wrap items-center justify-between gap-2 bg-black/20">
                        <p class="text-sm text-amber-100/70" x-text="battle.statusLine"></p>
